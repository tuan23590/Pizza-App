...
export default function ChiTietSanPham({ open, onClose, sanPham, mode }) {
  ...
  useEffect(() => {
    if (mode === "edit" && sanPham) {
      ...
      const kichThuocCheckedState = danhSachKichThuoc.reduce(
        (acc, kichThuoc, index) => {
          acc[index] = kichThuocParsed.some(
  ...
  const handleKichThuocChange = (index) => {
    ...
    setKichThuocChecked((prev) => {
      ...
      setFormData((prevData) => {
        // Đảm bảo danhSachKichThuoc luôn là một mảng
        const currentDanhSachKichThuoc = prevData.danhSachKichThuoc || [];

        let newDanhSachKichThuoc;

        if (newChecked) {
          newDanhSachKichThuoc = [
            ...currentDanhSachKichThuoc,
            { tenKichThuoc: kichThuoc, giaKichThuoc: 0 },
          ];
        } else {
          newDanhSachKichThuoc = currentDanhSachKichThuoc.filter(
            (item) => item.tenKichThuoc !== kichThuoc
          );
        }
  ...
  const handleDeChange = (index) => {
    ...
    setDeChecked((prev) => {
      ...
      setFormData((prevData) => {
        // Đảm bảo danhSachLoaiDe luôn là một mảng
        const currentDanhSachLoaiDe = prevData.danhSachLoaiDe || [];

        let newDanhSachLoaiDe;

        if (newChecked) {
          newDanhSachLoaiDe = [
            ...currentDanhSachLoaiDe,
            { tenLoaiDe: de, giaLoaiDe: 0 },
          ];
        } else {
          newDanhSachLoaiDe = currentDanhSachLoaiDe.filter(
  ...
  const handleDeGiaChange = (index, e) => {
    ...
    setFormData((prevData) => {
      const newDanhSachLoaiDe = prevData.danhSachLoaiDe.map((item) =>
        item.tenLoaiDe === de ? { ...item, giaLoaiDe: parseFloat(value) } : item
      );
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              value={
                formData?.giaSanPham
                  ? String(formData.giaSanPham).replace(
                      /\B(?=(\d{3})+(?!\d))/g,
                      ","
                    )
                  : ""
              ...
              <Grid item xs={6}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    variant="outlined"
                  >
                    <MenuItem value="Ngừng kinh doanh">
                      Ngừng kinh doanh
                    </MenuItem>
                    {formData?.soLuong > 0 && (
                      <MenuItem value="Đang kinh doanh">
                        Đang kinh doanh
                      </MenuItem>
                    )}
                  </Select>
          ...
          {(formData?.danhMuc === "DM1" ||
            formData?.danhSachKichThuoc?.length > 0 ||
            formData?.danhSachLoaiDe?.length > 0) && (
            <>
              <Grid item xs={6}>
                <FormGroup>
                  ...
                  {danhSachKichThuoc.map((kt, index) => (
                    ...
                    >
                      ...
                      <TextField
                        ...
                        value={
                          formData?.danhSachKichThuoc?.find(
                            (item) => item.tenKichThuoc === kt
                          )?.giaKichThuoc || ""
                        }
      ...
      <DialogActions>
        ...
        {formData?.danhMuc !== "DMXOA" &&
          (mode === "add" ? (
            <Button variant="contained" onClick={onSave} color="info">
              Lưu
            </Button>
          ) : (
            <Button variant="contained" onClick={onSave} color="info">
