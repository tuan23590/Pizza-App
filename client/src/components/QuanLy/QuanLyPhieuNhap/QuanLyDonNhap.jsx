          ...
          <Table>
            ...
            <TableBody>
              {danhSachDonNhap.map((donNhap) => (
                <TableRow key={donNhap.id} onClick={() => handleOpenDialog('edit', donNhap)} 
                sx={{ cursor: 'pointer', '&:hover': { backgroundColor: '#f5f5f5' } }}
                >
                  <TableCell>{donNhap.maDonNhap}</TableCell>
                  <TableCell>{donNhap.ngayNhap}</TableCell>
                  <TableCell>
                    {donNhap.chiTietDonNhap
                      .map((chiTietDonNhap) => chiTietDonNhap.nhaCungCap.tenNhaCungCap)
                      .join(', ')
                    }
                  </TableCell>
                  <TableCell>{donNhap.tongTien.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</TableCell>
                  <TableCell>{
                    donNhap.chiTietDonNhap
                      .map((chiTietDonNhap) => chiTietDonNhap.sanPham.tenSanPham)
                      .join(', ')  
                  }</TableCell>
