...
export default function TheoDoiDonHang() {
    ...
    const handleSearch = async () => {
        ...
        const sortedData = data.sort((a, b) => b.ngayDatHang - a.ngayDatHang).filter(donHang => donHang.trangThai == 'Đang xử lý' || donHang.trangThai == 'Đang chuẩn bị');
    ...
    return (
        <>
            <Typography fullWidth variant='h5' py={5} sx={{ textAlign: 'center' }}>THEO DÕI THỰC HIỆN ĐƠN HÀNG</Typography>
            <Box sx={{ marginX: 'auto', width: '33%' }}>
                <Paper sx={{ padding: '20px' }} elevation={3}>
                    {danhSachDonHang.length > 0 && (
                        <Box sx={{ marginX: 'auto' }}>
                            <Box>
                                {['Tất cả', 'Đang xử lý','Đang chuẩn bị'].map((trangThai, index) => (
                                    <Button
                                        key={index}
                                        sx={{ margin: '5px' }}
                                        variant="contained"
